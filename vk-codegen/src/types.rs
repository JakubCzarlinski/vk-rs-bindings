...
pub fn ctype_to_rust_str(ty: &CType) -> String {
    let base = {
        ...
        if mapped.is_empty() {
            if ty.base.is_empty() { "core::ffi::c_void".to_owned() }
            else { ty.base.clone() }
        } else {
            mapped.to_owned()
        }
    ...
    match ty.pointer_depth {
        0 => base,
        1 => if ty.is_const { format!("*const {base}") } else { format!("*mut {base}") },
        2 => if ty.is_const { format!("*const *const {base}") } else { format!("*mut *mut {base}") },
        _ => format!("*mut {base}"),
    }
}
...
pub fn const_rust_type(xml_type: &str, value: &str) -> &'static str {
    match xml_type.trim() {
        ...
        _ => {
            let v = value.trim();
            if v.ends_with('f') || v.ends_with('F') || v.contains('.') { "f32" }
