...
pub fn gen_queue_rs(reg: &Registry, result_cfgs: &HashMap<String, TokenStream>, handle_types: &HashSet<String>) -> String {
    let mut ts = TokenStream::new();
    ts.extend(preamble());
    ts.extend(gen_queue_dispatch_table(reg));
    ts.extend(gen_queue(reg, result_cfgs, handle_types));
    pretty(ts)
}

fn preamble() -> TokenStream {
    quote! {
        //! Queue-tier dispatch table and wrapper.
...
fn gen_queue_dispatch_table(reg: &Registry) -> TokenStream {
    let skip = entry_cmd_set();
    let enabled = enabled_set(reg);
    let groups = collect_groups(reg, Tier::Queue, &skip, &enabled);
    ...
    quote! {
        ...
        #[derive(Debug, Clone)]
        pub struct QueueDispatchTable { #fields_ts }
        #[cfg(feature = "VK_BASE_VERSION_1_0")]
...
fn gen_queue(reg: &Registry, result_cfgs: &HashMap<String, TokenStream>, handle_types: &HashSet<String>) -> TokenStream {
    let skip = entry_cmd_set();
    let enabled = enabled_set(reg);
    let groups = collect_groups(reg, Tier::Queue, &skip, &enabled);
    ...
    for cmds in groups.values() {
        for (name, providers, cmd) in cmds {
            methods_ts.extend(safe_method(cmd, name, providers, "VkQueue", quote!{ self.raw }, quote!{ &self.device.queue_table }, result_cfgs, handle_types));
        }
    }
    quote! {
        #[cfg(feature = "VK_BASE_VERSION_1_0")]
