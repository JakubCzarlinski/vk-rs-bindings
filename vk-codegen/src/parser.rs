        ...
        .filter(|n| n.is_element() && n.tag_name().name() == "type")
    {
        ...
        match category {
            ...
            "bitmask" => {
                ...
                let underlying = child_text(tn, "type").unwrap_or_else(|| "VkFlags".into());
                reg.typedefs.insert(
                    name.clone(),
                    Typedef {
                        name,
            ...
            "define" => {
                    ...
                    || raw.contains("variant")
                {
                    ...
                    let body: Option<&str> = if name.contains("MAKE_VIDEO_STD_VERSION") {
                        ...
                        Some(
                            "fn:variant,major,minor,patch|(variant << 29) | (major << 22) | (minor << 12) | patch",
                        )
                    ...
                    let is_commented =
                        raw.trim_start().starts_with("//") && !raw.contains(&"#define".to_string());
                    if is_commented {
                        None
                    } else {
                ...
                if ty_field.is_some() || ty_field.is_none() {
...
pub fn apply_require_extensions(reg: &mut Registry) {
    ...
    let disabled: std::collections::HashSet<String> = reg
        .extensions
        .iter()
    ...
    for td in reg.typedefs.values_mut() {
        if td.kind != TypedefKind::Define || !td.provided_by.is_empty() {
            continue;
        }
        if td.ty.is_none() {
            continue;
        } // no emittable content
